...
public class CustomIdentityStore implements IdentityStore {
    ...
    // R: This validates input crediential with exist crediential in DB
    @Override
    public CredentialValidationResult validate(Credential credential) {
        ...
        if (credential instanceof UsernamePasswordCredential) {
        	// R: callerName and credentialPassword is from the request header
            ...
            // R: retrive the security user from the DB
            SecurityUser user = jpaHelper.findUserByName(callerName);
            if (user != null) {
                String pwHash = user.getPwHash(); // R: retrive the encripted PW
                /*
                ...
                try {
                    ...
                    if (verified) {
                        Set<String> rolesForUser = jpaHelper.findRoleNamesForUser(callerName); // R: retrive the role of the username 
